...
- one-sentence generation, private preview interaction, immediate publication, Home discovery, and Feed playback;
- post-publication administrator takedown coverage in the API gate;
...
| Fixture moderation operator | `pulse.e2e.operator` | None. Used only by test setup with `X-Pulse-Admin` for post-publication takedown fixtures. | Isolated local development API only |
