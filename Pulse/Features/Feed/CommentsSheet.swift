...
struct CommentsSheet: View {
            ...
            VStack(spacing: 0) {
                ScrollView {
                    VStack(alignment: .leading, spacing: 14) {
                        ...
                        if model.comments(for: app.id).isEmpty {
                ...
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    HStack(alignment: .bottom, spacing: 10) {
                        ...
                        Button(action: submit) { Image(systemName: "arrow.up").font(.headline).frame(width: 40, height: 40) }
                            .buttonStyle(.borderedProminent).tint(.pulseLime).foregroundStyle(.black).disabled(draft.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
                    }
                }.padding(16)
            }
    ...
    private func submit() {
        ...
        model.addComment(to: app.id, body: text, score: score)
        draft = ""
    }
}
